      ...
      <aside className="hidden w-64 border-r border-stone-200 bg-white p-4 dark:border-stone-800 dark:bg-stone-950 lg:block">
        ...
        <nav className="space-y-1">
          {navItems.map(({ label, to, icon: Icon }) => (
            <NavLink key={to} to={to} end={to === '/admin'} className={({ isActive }) => cn('flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-stone-600 hover:bg-stone-100 dark:text-stone-300 dark:hover:bg-stone-900', isActive && 'bg-stone-950 text-white hover:bg-stone-950 dark:bg-white dark:text-stone-950 dark:hover:bg-white')}>

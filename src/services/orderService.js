...
export const orderService = {
  ...
  create(values, actor) {
    ...
    return database.transaction((state) => {
      ...
      const timestamp = now(); const items = values.items.map((item) => ({ ...item, id: item.id ?? database.createId('order-item'), status: item.status ?? 'pending' })); const subtotal = items.reduce((sum, item) => sum + item.price * item.quantity, 0); const tax = subtotal * ((state.settings?.taxRate ?? 0) / 100); const sequence = state.orders.length + 1001;
      const order = { ...values, items, id: database.createId('order'), table: table.name, waiterName: actor?.name ?? values.waiterName ?? 'Waiter', receiptNumber: `RCP-${new Date().getFullYear()}-${sequence}`, status: 'pending', paymentStatus: 'unpaid', subtotal, tax, discount: 0, total: subtotal + tax, billRequested: false, createdAt: timestamp, updatedAt: timestamp, timeline: [timeline('Order created', timestamp)] };

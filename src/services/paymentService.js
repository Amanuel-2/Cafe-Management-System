...
function complete(state, order, options, actor) {
  ...
  Object.assign(order, totals, { status: 'served', paymentStatus: 'paid', paymentMethod: options.method, collectedBy: actor?.name ?? 'Cashier', paidAt: timestamp, updatedAt: timestamp }); order.timeline.push({ id: database.createId('timeline'), title: 'Payment completed', timestamp, completed: true });
